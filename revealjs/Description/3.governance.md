...
Since the voting system uses a set of metrics (distance measure, consensus measure, proximit measure) to estimate the outcomes of a voting round, it alsos uses these metrics to provide feedback to voters. This feedback loop gives voers the ability to sense the extent to which they may need to adjust their preferences to align with the community to a greater degree. Arguably this can push the outcome towards a Pareto efficient outcome, under the assumptions they all voters are acting honestly. That being said, a malicious community member could also use it as a way to optimize towards selfish behavior. 
...
Since voting power is proportional to stake, then so are voter returns.

### Experts

As mentioned, Cardano uses a liquid democracy for their collective decision making. Experts are voters who also receive "delegated stake" from other voters, or experts. Delegated stake mechanics lets people hand over their voting power, as described in the liquid democracy model above.
...
Still, an expert could socially engineer a large set of voters to trust him/her and deviate from his/her "promises" if they are more knowledgeable, but in the long-run, such a tactic may not work as the likelihood of being caught is large. Indeed, it would be more rational to maintain a dominant position as a "more than average" knowledgeable person and reap consistent reweards, than to potentially reap high rewards once. Furthermore, there are checks in place in Voltaire to prevent this from happening since experts can be punished to the degree where they lose their security deposit / stake.
